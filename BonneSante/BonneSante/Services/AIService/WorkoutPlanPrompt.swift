...
"""
    ...
    static func coachUserPrompt(
        ...
        if let healthProfile, !healthProfile.isEmpty {
            text += """
            【用户健康与能量档案（App 内已知数据，请直接引用勿重复索要）】
            \(healthProfile)
    ...
    /// 训练计划页 AI 教练：本周排课 + 今日场次摘要
    static func planWeekCoachContext(
        entries: [WorkoutPlanEntry],
        exercisesBySession: [UUID: [WorkoutExercise]],
        phaseLabel: String,
        todayEntry: WorkoutPlanEntry?
    ) -> String {
        var lines = ["生理周期：\(phaseLabel)", "本周排课 \(entries.count) 场："]
        for entry in WorkoutPlanEntry.sortedMondayFirst(entries) {
            let exs = exercisesBySession[entry.id] ?? []
            let sets = exs.reduce(0) { $0 + max($1.sets, 1) }
            lines.append("- \(entry.weekdayLabel) \(entry.workoutType) · \(entry.targetMinutes)分钟 · 计划\(Int(entry.targetCalories))kcal · \(exs.count)动作/\(sets)组 · \(entry.isCompleted ? "已完成" : "未完成")")
        }
        if let today = todayEntry {
            let exs = exercisesBySession[today.id] ?? []
            lines.append("\n【今日场次详情】")
            lines.append(sessionSummary(entry: today, exercises: exs))
        } else {
            lines.append("\n今日无排课（休息日）")
        }
        return lines.joined(separator: "\n")
