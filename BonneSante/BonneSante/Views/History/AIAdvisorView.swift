...
struct AIAdvisorView: View {
    ...
    private func detectDataNeeds(from question: String) -> (needsWeight: Bool, needsFood: Bool, needsHealth: Bool, foodDays: Int) {
        let q = question.lowercased()

        let healthKeywords = ["体检", "报告", "指标", "胆固醇", "血糖", "尿酸", "结节", "风险", "异常", "复查", "ldl", "hdl"]
        let needsHealth = healthKeywords.contains { q.contains($0) }
        ...
        if !needsWeight && !needsFood && !needsHealth {
            return (true, true, true, 3)
        }

        return (needsWeight, needsFood, needsHealth, foodDays)
    }
    ...
    private func summaryForQuestion(_ question: String) -> String {
        ...
        if let context = healthContext {
            let overview = context.advisorContextSummary()
            if !overview.isEmpty {
                summaryLines.append("【今日概览】")
        ...
        if needs.needsHealth, let context = healthContext, let summary = context.healthSummary {
            summaryLines.append("【健康档案摘要】")
            summaryLines.append(summary.headline)
            for item in summary.abnormalItems.prefix(6) {
                summaryLines.append("· 异常：\(item)")
            }
            for note in summary.dietaryNotes {
                summaryLines.append("· 饮食注意：\(note)")
            }
            for risk in context.activeRiskFlags.prefix(5) {
                summaryLines.append("· 风险：\(risk.metricName) \(risk.currentValue) — \(risk.suggestedAction)")
            }
        }
                    ...
                    ScrollViewReader { proxy in
                        ScrollView {
                            LazyVStack(alignment: .leading, spacing: 12) {
                                // Welcome message
                                AIMessageBubble(content: "你好，我是 Bonne-Santé AI 健康顾问。\n\n我可以结合你的饮食记录、减脂目标与体检档案，提供专业的营养与健康建议。\n\n你可以问我：\n• 我的 LDL 偏高该怎么调整饮食？\n• 结合今日摄入，晚餐怎么安排？\n• 当前体检有哪些需要优先关注？")
    ...
    private var apiKeyPromptSection: some View {
        VStack(spacing: 20) {
            ...
            Text("AI 顾问需要 DeepSeek API 密钥才能使用。请先设置 API 密钥。")
                .font(.subheadline)
    ...
    private func askAI(question: String) async throws -> String {
        ...
        let systemPrompt = AdvisorPrompt.build(
            timeContext: currentTimeContext,
            dataSummary: dynamicSummary,
