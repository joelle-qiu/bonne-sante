...
struct FoodInputView: View {
    ...
    private var isAPIConfigured: Bool {
        // Text parsing needs DeepSeek; image needs Qwen
        ...
        NavigationStack {
            ...
            .sheet(isPresented: $showMultipleConfirmation) {
                MultipleFoodConfirmationView(
                    ...
                    initialMealType: resolvedMealTypeForBatch(),
    ...
    private var apiKeyPromptSection: some View {
        VStack(spacing: 20) {
            ...
            Text("请设置 DeepSeek API 密钥以启用文字解析功能。图片识别功能为可选。")
                .font(.subheadline)
    ...
    private func parseFood() async {
        ...
        if !APIKeyManager.isDeepSeekConfigured {
            errorMessage = "请先设置 DeepSeek API 密钥"
            showingAPIKeySetup = true
            return
        }
        ...
        do {
            ...
            if nutritionList.count == 1 {
                ...
                if let textMeal = MealType.inferredFromText(inputText) {
                    selectedMealType = textMeal
                }
    ...
    private func resolvedMealType(for nutrition: NutritionInfo) -> MealType {
        MealType.inferredFromText(inputText)
            ?? MealType.fromAIValue(nutrition.mealType)
            ?? selectedMealType
    }

    private func resolvedMealTypeForBatch() -> MealType {
        MealType.inferredFromText(inputText) ?? selectedMealType
    }
    ...
    private func saveFoodEntry(with nutrition: NutritionInfo) {
        ...
        let entry = FoodEntry(
            ...
            fat: nutrition.fat,
            date: entryDate,
            mealType: nutrition.resolvedMealType
        )
...
struct MultipleFoodConfirmationView: View {
    ...
    @State private var sessionMealType: MealType
    ...
    init(nutritionList: [NutritionInfo], initialMealType: MealType, onConfirm: @escaping ([NutritionInfo]) -> Void) {
        self.onConfirm = onConfirm
        self._editableList = State(initialValue: nutritionList)
        self._selectedItems = State(initialValue: Set(0..<nutritionList.count))
        self._sessionMealType = State(initialValue: initialMealType)
    }
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 8) {
                    ...
                    Text("总热量: \(Int(totalCalories)) kcal")
                        .font(.subheadline)
                        .foregroundStyle(.orange)
                    MealTypePicker(selection: $sessionMealType, compact: true)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("确认 (\(selectedItems.count))") {
                        ...
                        let confirmed = selectedItems.sorted().compactMap { index -> NutritionInfo? in
                            guard index < editableList.count else { return nil }
                            let item = editableList[index]
                            let meal = MealType.fromAIValue(item.mealType) ?? sessionMealType
                            return item.withMealType(meal)
                        }
                        onConfirm(confirmed)
                        dismiss()
                    }
