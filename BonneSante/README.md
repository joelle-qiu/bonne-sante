...
- **AI-Powered Nutrition Parsing**: Uses **DeepSeek API** (text) and Qwen VL Plus (image) for accurate calorie and macro estimation
...
- **DeepSeek API key** (required for text food parsing and AI advisor) — get at [platform.deepseek.com](https://platform.deepseek.com)
...
1. **Qwen Region** (for image recognition only):
   - **International**: DashScope international endpoint
   - **China**: DashScope China endpoint
...
### DeepSeek API (Text Parsing & AI Chat)
