...
> **截图顺序**：必须先 **生成/更新训练计划**，再验首页与营养 Tab。若先截首页后生成计划，首页会显示旧预算，**不算失败**——回首页下拉刷新即可。

| 顺序 | 路径 | 要点 | 场景 |
|------|------|------|------|
| 11 | **训练** → 训练计划 → 设频率并 **生成**（心情 2 次/周 或 专业 4 次/周） | 完成度 N/N、本周安排已落库 | 8.2、14.6 |
...
| 15 | 切 **首页** → **下拉刷新** → 再截图 | 能量环预算 = 当日训练/休息目标；洞察/日程与「本周安排」一致 | 6.5、6.7、6.8、8.8 |
...
| 6.8 | 已生成训练计划 · 今日无排课 | 自动切换休息日微调目标，首页进度条同步；**选非排课日或改系统日期补测** | ✅ `effectivePlan` + `NutritionMacroBars` | ☑ |

**备注**：6.7 / 6.8 真机已通过（2026-06-22 · 生成计划后回首页刷新；休息日 1530 kcal / +141 与营养 Tab 一致）。
